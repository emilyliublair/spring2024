        ...
        for i in range(len(arms)):
            if not(queue[i].empty()):
                ...
                if (status[i]):
                    ...
                    next_pos1[1] = snakings[i][0][snaking_tracker[i] % mods[i]] + finPosPopping[i][1]
                    next_pos1[3] = snakings[i][1][snaking_tracker[i] % mods[i]] + finPosPopping[i][3]
                    next_pos1[5] = snakings[i][2][snaking_tracker[i] % mods[i]] + finPosPopping[i][5]

                else:
                    thirdJVel = 0.0
                    next_pos1 = arms[i].angles
                    next_pos1[1] = snakings[i][0][snaking_tracker[i] % mods[i]] + initPosPopping[i][1]
                    next_pos1[3] = snakings[i][1][snaking_tracker[i] % mods[i]] + initPosPopping[i][3]
                    next_pos1[5] = snakings[i][2][snaking_tracker[i] % mods[i]] + initPosPopping[i][5]
