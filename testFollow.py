...
leftPopping = [0, 15.6, -46, 142.9, -10.1, 10.1, 35.2]
rightPopping = [0, 15.6, 63, 142.9, -10.1, 10.1, 35.2]
            ...
            if not(queue[i].empty()):
                ...
                if (status[i]):
                    ...
                    if coordinate[0] is not None and zone_2(coordinate[0], coordinate[1]):
                        ...
                        if head < -46: head = -46
                        if head > 63: head = 63
                        if (head > currentp[2] + followBuffer):
                            print(currentp)
                            next_pos[2] = currentp[2] + amt_to_move
                            thirdJVel = amt_to_move
                            print("right")
                            print(next_pos)
                        elif head < next_pos[2] - followBuffer:
                            next_pos[2] = currentp[2] - amt_to_move
                            thirdJVel = -amt_to_move
                            print("Left")
                        else: # head has tracked close enough -- prevent oscillations
                            print("close enough")
                            # do nothing
                    # return to center if no one detected
                    # else:
                    #     if next_pos[2] > 7.2:
                    #         if next_pos[2] - amt_to_move > 7.2:
                    #             thirdJVel = -amt_to_move
                    #         else:
                    #             thirdJVel = -(next_pos[2] - 7.2)
                    #         next_pos[2] = max(next_pos[2] - amt_to_move, 7.2)
                    #     elif next_pos[2] < 7.2:
                    #         if next_pos[2] + amt_to_move < 7.2:
                    #             thirdJVel = amt_to_move
                    #         else:
                    #             thirdJVel = 7.2 - next_pos[2]
                    #         next_pos[2] = min(next_pos[2] + amt_to_move, 7.2)
